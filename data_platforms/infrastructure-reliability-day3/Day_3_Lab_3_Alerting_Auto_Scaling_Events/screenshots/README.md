...
**Always include the region selector in AWS Console screenshots.** Steps 10–11 use your **email inbox** instead of the console.
...
|------|---------|------------------|----------------------|
| 1 | `Step_01_ASG_Verified.png` | **EC2** → **Auto Scaling Groups** → `WebServer-ASG` | Region **`us-east-1`** · Desired **2** / Min **2** / Max **6** · Activity: **Successful** launches · 2 **InService** instances |
| 2 | `Step_02_SNS_Topic.png` | **SNS** → **Topics** → `ASG-Alerts` | Region **`us-east-1`** · topic **`ASG-Alerts`** · **Topic ARN** (contains `:us-east-1:`) |
| 3 | `Step_03_Email_Subscription_Confirmed.png` | **SNS** → **Subscriptions** *or* email inbox | Status **Confirmed** *or* confirmation email from AWS |
| 4 | `Step_04_Scale_Out_Alarm.png` | **CloudWatch** → **Alarms** → `ASG-Scale-Out-Alert` | Region **`us-east-1`** · **> 2** threshold · notifies **ASG-Alerts** |
| 5 | `Step_05_Both_Alarms_OK.png` | **CloudWatch** → **Alarms** → **All alarms** | Both **`ASG-Scale-Out-Alert`** and **`ASG-Scale-In-Alert`** · state **OK** |
...
| 8 | `Step_08_CloudWatch_Dashboard.png` | **CloudWatch** → **Dashboards** → `ASG-Monitoring-Dashboard` | Region **`us-east-1`** · 3–4 widgets · ASG capacity metrics |
| 9 | `Step_09_Instance_Terminating.png` | **EC2** → **Instances** | Region **`us-east-1`** · one instance **Shutting-down** / **Terminated** · Instance ID |
| 10 | `Step_10_Terminate_Email_Alert.png` | **Email inbox** | Terminate or scale-in alarm email from AWS Notifications |
| 11 | `Step_11_Launch_Email_Alert.png` | **Email inbox** | Launch successful email · Instance ID in body |
